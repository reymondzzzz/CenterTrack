...
class GenericDataset(data.Dataset):
    ...
    def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
        ...
        self._input_preprocess = A.Compose([
            A.LongestMaxSize(max_size=self.opt.input_h, interpolation=3),
            A.PadIfNeeded(min_width=self.opt.input_w, min_height=self.opt.input_h,
                          value=(128, 128, 128), border_mode=0),
            # A.Normalize(mean=(0., 0., 0.), std=(1., 1., 1.), max_pixel_value=255.),
            # AP.transforms.ToTensorV2()
        ], bbox_params=A.BboxParams(format='pascal_voc', label_fields=['category_ids']))

        self._output_preprocess = A.Compose([
            A.LongestMaxSize(max_size=self.opt.output_h, interpolation=3),
            A.PadIfNeeded(min_width=self.opt.output_w, min_height=self.opt.output_h,
                          value=(128, 128, 128), border_mode=0),
            # A.Normalize(mean=(0., 0., 0.), std=(1., 1., 1.), max_pixel_value=255.),
            # AP.transforms.ToTensorV2()
        ], bbox_params=A.BboxParams(format='pascal_voc', label_fields=['category_ids']))

        self.augm_preprocess = A.Compose([
            A.OneOf([
                A.OneOf([
                    A.Downscale(scale_min=0.5, scale_max=0.5, p=0.3),
                    A.GaussianBlur(blur_limit=3),
                ], p=0.25),
                A.GaussNoise()
            ]),
            A.ChannelShuffle(),
            A.ShiftScaleRotate(shift_limit=0.03, scale_limit=(-0.1, 0), border_mode=cv2.BORDER_CONSTANT,
    ...
    def debug_img_bbox(self, img, bboxes, name):
        for b in bboxes:
            ...
            cv2.rectangle(img, tuple(xyxy[0:2]), tuple(xyxy[2:]), color)
    ...
    def __getitem__(self, index):
        ...
        output_bboxes = self._get_output_bboxes(img, anns)
        ...
        for k in range(num_objs):
            ...
            if cls_id <= 0 or ('iscrowd' in ann and ann['iscrowd'] > 0):
                self._mask_ignore_or_crowd(ret, cls_id, output_bboxes[k])
                continue
            self._add_instance(
                ret, gt_det, k, cls_id, output_bboxes[k], output_bboxes[k], ann, trans_output, aug_s,
    ...
    def _load_image_anns(self, img_id, coco, img_dir):
        ...
        # if ann['bbox'][2] > 12 or ann['bbox'][3] > 12
        anns = [ann for ann in anns if ann['bbox'][2] > 16 and ann['bbox'][3] > 16]
    ...
    def _get_pre_dets(self, anns, pre_bboxes):
        ...
        pre_cts, track_ids = [], []
        for ann, bbox in zip(anns, pre_bboxes):
            cls_id = int(self.cat_ids[ann['category_id']])
            ...
            if (h > 0 and w > 0):
                ...
                if 0:  # np.random.random() < self.opt.fp_disturb and reutrn_hm:
                    ct2 = ct0.copy()
    ...
    def resize_with_aspect_ratio(self, image, anns):
        ...
        # if img_w > img_h:
        #   new_w = self.opt.input_w
        #   ratio = float(img_w) / img_h
        #   new_h = new_w / ratio
        # else:
        #   new_h = self.opt.input_h
        #   ratio = float(img_h) / img_w
        #   new_w = new_h / ratio
        # new_w, new_h = int(new_w), int(new_h)
        # img = cv2.resize(image, (int(new_w), int(new_h)))
        # result = np.full((self.opt.input_h, self.opt.input_w, 3), (127, 127, 127), dtype=np.uint8)
        # x, y = int(self.opt.input_w / 2 - new_w / 2), int(self.opt.input_h / 2 - new_h / 2)
        # result[y:y+new_h, x:x+new_w] = img

        # bboxes = []
        # for ann in anns:
        #   xywh = ann['bbox']
        #   xyxy = [
        #     xywh[0] / img_w             * ratio - x / self.opt.input_w,
        #     xywh[1] / img_h             * ratio - y / self.opt.input_h,
        #     (xywh[0] + xywh[2]) / img_w * ratio - x / self.opt.input_w,
        #     (xywh[1] + xywh[3]) / img_h * ratio - y / self.opt.input_h
        #   ]
        #   bboxes.append(xyxy)
        #
        # for b in bboxes:
    ...
    def _get_input(self, img, trans_input, anns):
